...
def pdbSeq(pdbfile,use_atoms=False):
    """
    ...
    # Try using SEQRES
    seq = [l for l in pdbfile if l[0:6] == "SEQRES"]
    ...
    # Otherwise, use ATOM
        ...
        # at the first model.
        models = [i for i, l in enumerate(pdbfile) if l.startswith("MODEL")]
        if len(models) > 1:
            pdbfile = pdbfile[models[0]:models[1]]     
...
def convertModifiedAA(chain_dict,pdbfile):
    """
    ...
    # are not, return
    modres = [l for l in pdbfile if l[0:6] == "MODRES"]
    if len(modres) == 0:
        return chain_dict
...
def pdbSeq2Fasta(pdbfile,pdb_id="",chain="all",use_atoms=False):
    """
    Extract sequence from pdb file and write out in FASTA format.
    """

    # Grab sequences
    chain_dict, seq_type = pdbSeq(pdbfile,use_atoms)

    # Convert modified amino acids to their natural counterparts
    chain_dict = convertModifiedAA(chain_dict,pdbfile)
